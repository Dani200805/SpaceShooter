import { Player, Bullet, Explosion, PowerUp, Enemy } from "./classes.js"
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initGame() {
        ...
        enemies = createEnemies(TARGET_FPS, 5, canvas.width, 40, gameImages.enemy)
    ...
    function shoot() {
        ...
        bullets.push(new Bullet(bulletX, bulletY, bulletWidth, bulletHeight, "#FFFF00", 10  * TARGET_FPS, bulletImage, currentBulletType))
    }
    ...
    */
    function handleShooting(deltaTime) {
        ...
        if (shootCooldown > 0) {
            shootCooldown -= deltaTime * TARGET_FPS
        }

        if ((keys[" "] || activePowerUps.autoShoot.active) && shootCooldown <= 0) {
            shoot()

            if (activePowerUps.fireRateUpgrade.active) { 
                shootCooldown = 5
            } else { 
                shootCooldown = 10 
            }
    ...
    /**
     * @returns {boolean} true if the bullet type has reached the maximum level
     */
    ...
    function applyPowerUpEffect(powerUpType) {
        switch (powerUpType) {
            ...
            case "autoShoot":
                ...
                activePowerUps.autoShoot.duration += 2500
                break
        }
    }
     ...
     * @param {number} deltaTime
     */
    function updatePowerUpDurations(deltaTime) {
        for (const [type, powerUp] of Object.entries(activePowerUps)) {
            if (powerUp.active) {
                powerUp.duration -= deltaTime * TARGET_FPS
    ...
    function update(deltaTime) {
        ...
        enemies.forEach((enemy) => {
            if (enemy.update(deltaTime, canvas.height)) {
                enemyReachedBottom = true
            }
        })

        powerUps = powerUps.filter((powerUp) => powerUp.update(deltaTime, canvas.height))
        ...
        enemySpawnCounter += deltaTime * TARGET_FPS
        if (enemySpawnCounter >= enemySpawnRate) {
            enemies = enemies.concat(createEnemies(TARGET_FPS, maxEnemyCount - enemies.length, canvas.width, 40, gameImages.enemy))
            enemySpawnCounter = 0
        ...
        if (score - lastPowerUpScore >= 60 * maxEnemyCount || Math.random() < powerUpSpawnChance * deltaTime * TARGET_FPS) {
            powerUps.push(createPowerUp(TARGET_FPS, canvas.width, 30, gameImages.powerUp))
        ...
        if (player.isActive) {
            powerUps.forEach((powerUp) => {
                if (player.collidesWith(powerUp)) {
                    const powerUpType = powerUp.applyEffect()
            ...
            explosions = explosions.filter((explosion) => explosion.update(deltaTime))
        }
    }
    ...
    function drawPowerUpIndicators() {
        ...
        for (const [type, powerUp] of Object.entries(activePowerUps)) {
            if (powerUp.active) {
                ...
                ctx.fillText(
                    `${type}: ${Math.ceil(powerUp.duration / TARGET_FPS)}s`,
